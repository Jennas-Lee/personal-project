...
<body>
    ...
    <main role="main">
        <?php
            if(isset($_GET['title'])) {
                $title = $_GET['title'];
                $sql = "SELECT * FROM CLASS_TB WHERE title='{$title}'";
                $result = mysqli_query($conn, $sql);
                $row = mysqli_fetch_array($result);
                if($row == NULL) {
                    echo '<div class="container mt-4"><a href="timetable.php">반 이름이 잘못되었습니다. 다시 선택해주세요.</a> 선택한 반 : '.$title.'</div>';
                } else {
        ?>
        ...
        <div class="container">
            <table id="memberTable" class="table table-striped text-center tablesorter mt-4">
                ...
                <tbody>
                    <?php
                        $sql = "SELECT * FROM STUDENT_TB WHERE class='{$title}' ORDER BY name ASC";
                        $result = mysqli_query($conn, $sql);
        ...
        <?php
                ...
                echo '<div class="container mt-4"><a href="timetable.php">반을 선택해주세요.</a></div>';
            }
        ?>
    </main>
    <footer>
